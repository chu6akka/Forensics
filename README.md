...
- Для контекстного морфологического анализа используется `Natasha`
  (нейросетевая модель), что помогает различать омонимичные формы по контексту.
- Если модель `Natasha` недоступна, используется резервный анализ `pymorphy3`,
