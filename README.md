...
- выводит список всех встреченных словоформ с указанием части речи;
- экспортирует результат в Excel.
- считает частотные коэффициенты частей речи и отображает диаграмму.
...
- Для морфологического анализа используется `pymorphy3`, который поддерживает
  категории русского языка (включая деепричастия и причастия).
